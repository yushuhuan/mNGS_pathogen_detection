...
### 提取species水平的病原信息
species_reads_info <- classified_nt %>% filter(grepl("^S", tax_level)) %>% select(name, tax_id, all_reads)
...
# 过滤掉人的分类信息
output <- output %>% filter(tax_id != "9606") %>% filter(tax_id != "9598")  %>% select(Domain, Genius, Species, tax_id, all_reads)
...
output <- output %>% select(Rank, tax_id, Domain, Genius, Species, all_reads, aboundance_based_domain, aboundance_based_species)
output$aboundance_based_domain <- as.vector(output$aboundance_based_domain)
str(output)

write.table(output, paste0(output_dir, "/pathogen_with_genius_and_domain.txt"), row.names = F, sep = "\t", quote = F)
