...
### 获取致病性及常见背景微生物label
concerned_pathogens_V1 <- read.table("/opt/mNGS/run2/known_pathogen_background_V1.0.txt", header = T, sep = "\t", fill = TRUE)
...
bacteria_attention <- function(species, reads_num, rank, genius, aboundance_based_species, reads_in_NC){
    ...
    if(species %in% known_species){
        ...
        if(species %in% c("Pseudomonas aeruginosa", "Acinetobacter baumannii")){if(reads_num > 100){attention_level = "高关注"}else{if(reads_num < 20){attention_level = "低关注"}}}
    }else {
...
fungi_attention <- function(species, rank, genius){
    known_species = c("Komagataella pastoris", "Candida orthopsilosis")
    known_genius = c("Candida", "Aspergillus", "Fusarium", "Malassezia")
    if(species %in% c(known_species)){
        if(species == "Komagataella pastoris"){attention_level = "污染"}
        if(species == "Candida orthopsilosis"){attention_level = ""}
    }else {
       if(genius %in% known_genius){
        if(genius %in% c("Candida", "Aspergillus", "Fusarium")){
            if(rank <= 3){attention_level = "高关注"}else{attention_level = ""}
        }else {
           attention_level = ""
        }
       }else {
...
extracted_result <- extracted_result %>% select(attention_level, speciescn, all_reads, Rank, Domain, Genius, Species, aboundance_based_domain, aboundance_based_species, Reads_num_in_NC, label)
colnames(extracted_result)[10] <- "label_experience"

str(extracted_result)
write.table(extracted_result, paste0(output_dir, "/pathogen_report.txt"), row.names = F, sep = "\t", quote = F)
